...
Spotlight 不要求乘客在拥挤的上车点阅读大量信息，而是把识别任务分配到多个通道中：AR 帮助用户定位被分配的车辆，车外灯光提供远距离识别，声音则在上车、下车和行人警告等关键时刻提供即时反馈。
...
有三个因素会影响用户看到的灯光颜色：环境颜色、灯光亮度、灯光设备的色偏。所以我们去掉了可能与黄色混淆的橙色，以及可能与靛色混淆的蓝色。当同一地点出现大量公交车、单一颜色不足以区分时，系统也可以使用双色组合来扩展识别数量。
